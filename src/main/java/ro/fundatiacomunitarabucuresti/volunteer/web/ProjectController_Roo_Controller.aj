...
privileged aspect ProjectController_Roo_Controller {
    ...
    @Autowired
    OrganizationService ProjectController.organizationService;
    
    @Autowired
    CityService ProjectController.cityService;
    
    @Autowired
    ProjectService ProjectController.projectService;
    
    @Autowired
    SkillService ProjectController.skillService;
    
    @Autowired
    TagService ProjectController.tagService;
    
    @Autowired
    VolunteerApplicationService ProjectController.volunteerApplicationService;
    ...
    public String ProjectController.createForm(Model uiModel) {
        ...
        if (organizationService.countAllOrganizations() == 0) {
            dependencies.add(new String[] { "organization", "organizations" });
        }
        if (cityService.countAllCitys() == 0) {
            dependencies.add(new String[] { "city", "cities" });
        }
    ...
    void ProjectController.populateEditForm(Model uiModel, Project project) {
        ...
        uiModel.addAttribute("volunteerapplications", volunteerApplicationService.findAllVolunteerApplications());
    }
